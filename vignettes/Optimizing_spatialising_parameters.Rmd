...
The **spatialising** package allows to perform simulations of binary spatial raster data using the Ising model.
This model requires two parameters: `B` and `J`.
The goal of this document is to show how to optimize these parameters to obtain the best possible simulation.
...
A careful look at the plot above reveals an increase in the forested area between 2013 and 2016.

## Metrics

For each of our rasters, we calculate two metrics: magnetization and texture index.
...
Here, we can see an increase in the values of magnetization between 2013 and 2016, which means that the forested area (`1`) had increased.
...
Now, we could try to simulate the state in the year 2016 using the `maine2013` raster as a reference with the `spatial_ising()` function.
...
For example, negative `B` results in a higher probability of a cell to change its value to `-1`, while positive `B` gives a higher probability of a cell changing its value to `1`.
The negative `J` parameter results in a higher probability of a cell to change its value to the opposite value of its neighbors, while positive `J` gives a higher probability of a cell to change its value to the value of its neighbors.
...
```{r}
sim1 = spatial_ising(maine2013, B = 0.3, J = 0.9, iter = ncell(maine2013) * 3, inertia = 150)
sim2 = spatial_ising(maine2013, B = 0.7, J = 0.1, iter = ncell(maine2013) * 3, inertia = 150)
```
...
In this case, the best combination found is `B = 0.08` and `J = 0.47`.^[You just need to be aware that simulated annealing is a stochastic algorithm, so the results may be slightly different each time you run it.]
...
Finally, we can compare the rasters for the years 2013, 2016, and the simulation for 2016.
