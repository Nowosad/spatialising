...
For each of our rasters, we calculate two metrics: magnetization and texture index. 
The magnetization ($m$) is the sum of all raster values divided by the number of cells.
The name "magnetization" is historical as the Ising model was originally developed as a theoretical model of magnetized substances.
It has a range from `-1` to `1`, where `-1` means that all cells have value `-1` and `1` means that all cells have value `1`.
...
`J` (always positive) is an interaction between neighboring cells: it tries to align signs of neighboring cells.  
...
Then, a the Euclidean "distance" between two patterns in terms of these two metrics is *[(metricA1-metricB1)^2 +{metricB1-metricB2)^2]^(1/2)*.
...
optim_params = optim_sa(fun = optimize_model, 
                        start = c(0, 0), lower = c(-0.9, 0), upper = c(0.9, 0.9),trace = TRUE)
```
...
```{r}
sim_optim_metrics = c(magnetization(sim_optim), texture_index(sim_optim))             
